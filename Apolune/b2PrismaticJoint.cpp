...
void b2PrismaticJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	// Prismatic constraint.
	{
		...
		b2decimal m1 = m_invMassA, m2 = m_invMassB;
		b2decimal i1 = m_invIA, i2 = m_invIB;

		b2decimal k11 = m1 + m2 + i1 * m_s1 * m_s1 + i2 * m_s2 * m_s2;
		b2decimal k12 = i1 * m_s1 + i2 * m_s2;
		b2decimal k13 = i1 * m_s1 * m_a1 + i2 * m_s2 * m_a2;
		b2decimal k22 = i1 + i2;
		b2decimal k23 = i1 * m_a1 + i2 * m_a2;
		b2decimal k33 = m1 + m2 + i1 * m_a1 * m_a1 + i2 * m_a2 * m_a2;
	...
	if (m_enableLimit)
	{
		b2decimal jointTranslation = b2Dot(m_axis, d);
	...
	if (step.warmStarting)
	{
		...
		b2decimal L1 = m_impulse.x * m_s1 + m_impulse.y + (m_motorImpulse + m_impulse.z) * m_a1;
		b2decimal L2 = m_impulse.x * m_s2 + m_impulse.y + (m_motorImpulse + m_impulse.z) * m_a2;
...
void b2PrismaticJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal w1 = b1->m_angularVelocity;
	b2Vec2 v2 = b2->m_linearVelocity;
	b2decimal w2 = b2->m_angularVelocity;

	// Solve linear motor constraint.
	if (m_enableMotor && m_limitState != e_equalLimits)
	{
		b2decimal Cdot = b2Dot(m_axis, v2 - v1) + m_a2 * w2 - m_a1 * w1;
		b2decimal impulse = m_motorMass * (m_motorSpeed - Cdot);
		b2decimal oldImpulse = m_motorImpulse;
		b2decimal maxImpulse = step.dt * m_maxMotorForce;
		m_motorImpulse = b2Clamp(m_motorImpulse + impulse, -maxImpulse, maxImpulse);
		impulse = m_motorImpulse - oldImpulse;

		b2Vec2 P = impulse * m_axis;
		b2decimal L1 = impulse * m_a1;
		b2decimal L2 = impulse * m_a2;
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		b2decimal L1 = df.x * m_s1 + df.y + df.z * m_a1;
		b2decimal L2 = df.x * m_s2 + df.y + df.z * m_a2;
	...
	else
	{
		...
		b2decimal L1 = df.x * m_s1 + df.y;
		b2decimal L2 = df.x * m_s2 + df.y;
...
bool b2PrismaticJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	b2decimal a1 = b1->m_sweep.a;

	b2Vec2 c2 = b2->m_sweep.c;
	b2decimal a2 = b2->m_sweep.a;

	// Solve linear limit constraint.
	b2decimal linearError = 0.0, angularError = 0.0;
	bool active = false;
	b2decimal C2 = 0.0;
	...
	if (m_enableLimit)
	{
		...
		b2decimal translation = b2Dot(m_axis, d);
		if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2.0 * b2_linearSlop)
		{
	...
	b2decimal L1 = impulse.x * m_s1 + impulse.y + impulse.z * m_a1;
	b2decimal L2 = impulse.x * m_s2 + impulse.y + impulse.z * m_a2;
...
b2Vec2 b2PrismaticJoint::GetReactionForce(b2decimal inv_dt) const
{
	return inv_dt * (m_impulse.x * m_perp + (m_motorImpulse + m_impulse.z) * m_axis);
}

b2decimal b2PrismaticJoint::GetReactionTorque(b2decimal inv_dt) const
{
	return inv_dt * m_impulse.y;
}

b2decimal b2PrismaticJoint::GetJointTranslation() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
	...
	b2decimal translation = b2Dot(d, axis);
	return translation;
}

b2decimal b2PrismaticJoint::GetJointSpeed() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
	...
	b2decimal speed = b2Dot(d, b2Cross(w1, axis)) + b2Dot(axis, v2 + b2Cross(w2, r2) - v1 - b2Cross(w1, r1));
	return speed;
}
...
b2decimal b2PrismaticJoint::GetLowerLimit() const
{
	return m_lowerTranslation;
}

b2decimal b2PrismaticJoint::GetUpperLimit() const
{
	return m_upperTranslation;
}

void b2PrismaticJoint::SetLimits(b2decimal lower, b2decimal upper)
{
...
void b2PrismaticJoint::SetMotorSpeed(b2decimal speed)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
	m_motorSpeed = speed;
...
void b2PrismaticJoint::SetMaxMotorForce(b2decimal force)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
...
b2decimal b2PrismaticJoint::GetMotorForce() const
{
