...
void b2World::SolveTOI(b2Body* body)
{
	...
	const b2decimal k_toiBaumgarte = 0.75f;
	bool solved = false;
	for (int i = 0; i < 20; ++i)
	{
...
void b2World::Step(b2decimal dt, int velocityIterations, int positionIterations)
{
...
{
	b2decimal RayCastCallback(const b2RayCastInput& input, int proxyId)
	{
		...
		if (hit)
		{
			b2decimal fraction = output.fraction;
			b2Vec2 point = (1.0 - fraction) * input.p1 + fraction * input.p2;
	...
	case b2Shape::e_circle:
		{
			...
			b2decimal radius = circle->m_radius;
