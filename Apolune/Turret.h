/*******************************************************************************************************************************\
| 88888888888          88                  LOST ASTRONAUT'S                 * Lost Astronaut Game Development Framework         |
| 88                   ""                                                   * Copyright (c) 2007-2017 H. Elwood Gilliland III   |
| 88aaaaa  8b,dPPYba,  88  8b,dPPYba,    ,adPPYb,d8   ,adPPYba,  ,adPPYba,  ****************************************************|
| 88UUUUU  88P'   "Y8  88  88P'   `"8a  a8"    `Y88  a8P_____88  I8[    ""  * This software is copyrighted software.            |
| 88       88          88  88       88  8b       88  8PP"""""""   `"Y8ba,   * Use of this code is given only with permission    |
| 88       88          88  88       88  "8a,   ,d88  "8b,   ,aa  aa    ]8I  * to parties who have been granted such permission  |
| 88       88          88  88       88   `"YbbdP"Y8   `"Ybbd8"'  `"YbbdP"'  * by its author, Herbert Elwood Gilliland III.      |
|    O  F      T H E     E M P I R E     aa,    ,88                         * All rights reserved.                              |
|_______________________________________  "Y8bbdP"   __________________________________________________________________________*/
#pragma once

#include "GLWindow.h"
#include "Sprite.h"

// Munition type
SINGLE(Ordinance,{});
 Zp<Sprite> image;
 Zdouble range,force,damage;
PLURAL(Ordinance,Ordinances,{});
ENDSET(Ordinances);

struct Ammunition {
public:
 Zp<Ordinance> ordinance;
 Zint loadable,perMag,remaining;
};

SINGLE(MuzzleFlash,{});
PLURAL(MuzzleFlash,MuzzleFlashes,{});
ENDSET(MuzzleFlashes);

SINGLE(TurretFiringMode,{});
PLURAL(TurretFiringMode,TurretFiringModes,{});
ENDSET(TurretFiringModes);

// Current events
struct TurretProfile {
public:
 Zdouble facing,restAngle,damage; // facingAngle, restAngle, damage received
 Ammunition ammo;
};

SINGLE(Turret,{});
 TurretProfile profile;
 Zp<GLImage> image;
 Cartesian offset;
 Zdouble turnableAngle;
 Zint cost; // in 'credits'
 Zdouble rate,strength; // Range of bullet, force/speed, damage dealt, rate of fire, strength (health) of item
PLURAL(Turret,Turrets,{});
ENDSET(Turrets);

SINGLE(AttachmentPoint,{});
PLURAL(AttachmentPoint,AttachmentPoints,{});
ENDSET(AttachmentPoints);