...
void b2LineJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	// Prismatic constraint.
	{
		...
		b2decimal k12 = i1 * m_s1 * m_a1 + i2 * m_s2 * m_a2;
		b2decimal k22 = m1 + m2 + i1 * m_a1 * m_a1 + i2 * m_a2 * m_a2;
	...
	if (step.warmStarting)
	{
		...
		b2decimal L1 = m_impulse.x * m_s1 + (m_motorImpulse + m_impulse.y) * m_a1;
		b2decimal L2 = m_impulse.x * m_s2 + (m_motorImpulse + m_impulse.y) * m_a2;
...
void b2LineJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal Cdot1 = b2Dot(m_perp, v2 - v1) + m_s2 * w2 - m_s1 * w1;

	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		// Solve prismatic and limit constraint in block form.
		b2decimal Cdot2 = b2Dot(m_axis, v2 - v1) + m_a2 * w2 - m_a1 * w1;
		...
		// f2(1) = invK(1,1) * (-Cdot(1) - K(1,2) * (f2(2) - f1(2))) + f1(1)
		b2decimal b = -Cdot1 - (m_impulse.y - f1.y) * m_K.col2.x;
		b2decimal f2r;
		if (m_K.col1.x != 0.0)
		{
		...
		b2decimal L1 = df.x * m_s1 + df.y * m_a1;
		b2decimal L2 = df.x * m_s2 + df.y * m_a2;
	...
	else
	{
		...
		b2Vec2 P = df * m_perp;
		b2decimal L1 = df * m_s1;
		b2decimal L2 = df * m_s2;
...
bool b2LineJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	b2decimal L1 = impulse.x * m_s1 + impulse.y * m_a1;
	b2decimal L2 = impulse.x * m_s2 + impulse.y * m_a2;
...
b2Vec2 b2LineJoint::GetReactionForce(b2decimal inv_dt) const
{
	return inv_dt * (m_impulse.x * m_perp + (m_motorImpulse + m_impulse.y) * m_axis);
}

b2decimal b2LineJoint::GetReactionTorque(b2decimal inv_dt) const
{
	B2_NOT_USED(inv_dt);
	return 0.0;
}

b2decimal b2LineJoint::GetJointTranslation() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
...
b2decimal b2LineJoint::GetJointSpeed() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
...
b2decimal b2LineJoint::GetLowerLimit() const
{
	return m_lowerTranslation;
}

b2decimal b2LineJoint::GetUpperLimit() const
{
	return m_upperTranslation;
}

void b2LineJoint::SetLimits(b2decimal lower, b2decimal upper)
{
...
void b2LineJoint::SetMotorSpeed(b2decimal speed)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
	m_motorSpeed = speed;
...
void b2LineJoint::SetMaxMotorForce(b2decimal force)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
...
b2decimal b2LineJoint::GetMotorForce() const
{
