...
{
    FILE* fp = 0;
    bstring effectFile=0, effectContents=0;
...
int i;
	...
	if (len > 0 && len < b->mlen ) bstr__memcpy (b->data, blk, (size_t) len);
...
struct tagbstring x;
	...
	if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0 || r->slen < 0 || r->mlen < r->slen) return BSTR_ERR;
	l = s->buff->slen;
...
#define exvsnprintf(r,b,n,f,a) {r = _vsnprintf_s(b,n,n-1,f,a);}
#else
