...
public:
 ...
 float center[2];
 GLuint *pTex0;
 GLint Uradius,Uangle,Uresolution,Ucenter,Utex0;
 GLuint Utrans;
 ...
 void Init() {
  ...
  string frag=
   ...
   string("uniform sampler2D tex;  // Scene buffer\n")+
   ...
   string("  vec2 tc = Texcoord * texSize;\n")+
   string("  tc -= center;\n")+
   ...
   string("  }\n")+
   string("  tc += center;\n")+
   string("  vec3 color = texture2D(tex, tc / texSize).rgb;\n")+
   string("  return vec4(color, 1.0);\n")+
   string("}\n")+
   string("void main (void)\n")+
   string("{\n")+
   string("  outColor = PostFX();\n")+
   string("}\n")
  ;
