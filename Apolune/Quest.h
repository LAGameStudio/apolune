...
ONE(Quest,{})
 ...
 Zstring named;
 Zstring reward; // script
 Zstring required; // space separated scripts, prerequisite quest refs
 Zstring hint; // hint / description
 Zstring locationHint; // allows you to hint at a starmap location on the starmap
 KEYWORDS({
  TXTWORD("name",name)
  else TXTWORD("named",named)
  else TXTWORD("reward",reward)
  else TXTWORD("hint",hint)
  else TXTWORD("required",required)
  else TXTWORD("locationHint",locationHint)
 })
 WORDKEYS({
  TXTKEY("name",name)
  TXTKEY("named",named)
  TXTKEY("reward",reward)
  TXTKEY("hint",hint)
  TXTKEY("required",required)
  TXTKEY("locationHint",locationHint)
 })
MANY(Quest,QuestHandle,QuestHandles,"quest",Quests,{})
 KEYSWORDSGroups("quest",Quest)
 void AssignUnassigned( Campaign *c );
 void Query( Campaign *c, QuestHandles *out );
DONE(Quest);
