...
CL::CL()
{
    OUTPUT("Initialize OpenCL object and context\n");
    //setup devices and context
    std::vector<cl::Platform> platforms;
    err = cl::Platform::get(&platforms);
    OUTPUT("cl::Platform::get(): %s\n", oclErrorString(err));
    OUTPUT("platforms.size(): %d\n", platforms.size());

    deviceUsed = 0;
    err = platforms[0].getDevices(CL_DEVICE_TYPE_GPU, &devices);
    OUTPUT("getDevices: %s\n", oclErrorString(err));
    OUTPUT("devices.size(): %d\n", devices.size());
    int t = devices.front().getInfo<CL_DEVICE_TYPE>();
    OUTPUT("type: device: %d CL_DEVICE_TYPE_GPU: %d \n", t, CL_DEVICE_TYPE_GPU);
...
void CL::loadProgram(std::string kernel_source)
{
 // Program Setup
    ...
    OUTPUT("load the program\n");
    
    pl = kernel_source.size();
    OUTPUT("kernel size: %d\n", pl);
    //printf("kernel: \n %s\n", kernel_source.c_str());
    try
    {
    ...
    try
    {
        ...
        OUTPUT("program.build: %s\n", oclErrorString(er.err()));
    //if(err != CL_SUCCESS){
    }
    OUTPUT("done building program\n");
