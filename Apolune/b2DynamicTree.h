...
inline void b2DynamicTree::RayCast(T* callback, const b2RayCastInput& input) const
{
	...
	b2Vec2 v = b2Cross(1.0, r);
	b2Vec2 abs_v = b2Abs(v);

	// Separating axis for segment (Gino, p80).
	// |dot(v, p1 - c)| > dot(|v|, h)

	b2decimal maxFraction = input.maxFraction;
	...
	while (count > 0)
	{
		...
		b2decimal separation = b2Abs(b2Dot(v, p1 - c)) - b2Dot(abs_v, h);
		if (separation > 0.0)
		{
			continue;
		}
		...
		if (node->IsLeaf())
		{
			...
			b2decimal value = callback->RayCastCallback(subInput, nodeId);
