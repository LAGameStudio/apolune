...
b2Body::b2Body(const b2BodyDef* bd, b2World* world)
{
	...
	b2Assert(b2IsValid(bd->inertiaScale) && bd->inertiaScale >= 0.0);
...
b2Fixture* b2Body::CreateFixture(const b2Shape* shape, b2decimal density)
{
	b2FixtureDef def;
	def.shape = shape;
	def.density = density;
...
void b2Body::SetMassData(const b2MassData* massData)
{
	...
	if (massData->I > 0.0 && (m_flags & b2Body::e_fixedRotationFlag) == 0)
	{
...
void b2Body::SetTransform(const b2Vec2& position, b2decimal angle)
{
	b2Assert(m_world->IsLocked() == false);
	if (m_world->IsLocked() == true)
	{
