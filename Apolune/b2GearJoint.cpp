...
: b2Joint(def)
{
	...
	b2decimal coordinate1, coordinate2;
...
void b2GearJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal Cdot = m_J.Compute(	b1->m_linearVelocity, b1->m_angularVelocity,
								b2->m_linearVelocity, b2->m_angularVelocity);

	b2decimal impulse = m_mass * (-Cdot);
	m_impulse += impulse;
...
bool b2GearJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	b2decimal C = m_constant - (coordinate1 + m_ratio * coordinate2);
...
b2Vec2 b2GearJoint::GetReactionForce(b2decimal inv_dt) const
{
...
b2decimal b2GearJoint::GetReactionTorque(b2decimal inv_dt) const
{
	// TODO_ERIN not tested
	b2Vec2 r = b2Mul(m_bodyB->GetTransform().R, m_localAnchor2 - m_bodyB->GetLocalCenter());
	b2Vec2 P = m_impulse * m_J.linearB;
	b2decimal L = m_impulse * m_J.angularB - b2Cross(r, P);
	return inv_dt * L;
}

void b2GearJoint::SetRatio(b2decimal ratio)
{
	b2Assert(b2IsValid(ratio));
	m_ratio = ratio;
}

b2decimal b2GearJoint::GetRatio() const
{
	return m_ratio;
}
