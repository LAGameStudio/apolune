...
void b2DistanceJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal cr1u = b2Cross(r1, m_u);
	b2decimal cr2u = b2Cross(r2, m_u);
	b2decimal invMass = b1->m_invMass + b1->m_invI * cr1u * cr1u + b2->m_invMass + b2->m_invI * cr2u * cr2u;

	m_mass = invMass != 0.0 ? 1.0 / invMass : 0.0;

	if (m_frequencyHz > 0.0)
	{
		b2decimal C = length - m_length;

		// Frequency
		b2decimal omega = 2.0 * b2_pi * m_frequencyHz;

		// Damping coefficient
		b2decimal d = 2.0 * m_mass * m_dampingRatio * omega;

		// Spring stiffness
		b2decimal k = m_mass * omega * omega;
...
void b2DistanceJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal Cdot = b2Dot(m_u, v2 - v1);

	b2decimal impulse = -m_mass * (Cdot + m_bias + m_gamma * m_impulse);
	m_impulse += impulse;
...
bool b2DistanceJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	b2decimal length = d.Normalize();
	b2decimal C = length - m_length;
	C = b2Clamp(C, -b2_maxLinearCorrection, b2_maxLinearCorrection);

	b2decimal impulse = -m_mass * C;
	m_u = d;
	b2Vec2 P = impulse * m_u;
...
b2Vec2 b2DistanceJoint::GetReactionForce(b2decimal inv_dt) const
{
	b2Vec2 F = (inv_dt * m_impulse) * m_u;
	return F;
}

b2decimal b2DistanceJoint::GetReactionTorque(b2decimal inv_dt) const
{
	B2_NOT_USED(inv_dt);
	return 0.0;
}
