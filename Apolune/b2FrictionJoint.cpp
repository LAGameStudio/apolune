...
void b2FrictionJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal mA = bA->m_invMass, mB = bB->m_invMass;
	b2decimal iA = bA->m_invI, iB = bB->m_invI;
...
void b2FrictionJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal wA = bA->m_angularVelocity;
	b2Vec2 vB = bB->m_linearVelocity;
	b2decimal wB = bB->m_angularVelocity;
	...
	{
		b2decimal Cdot = wB - wA;
		b2decimal impulse = -m_angularMass * Cdot;

		b2decimal oldImpulse = m_angularImpulse;
		b2decimal maxImpulse = step.dt * m_maxTorque;
	...
	// Solve linear friction
	{
		...
		b2decimal maxImpulse = step.dt * m_maxForce;
...
bool b2FrictionJoint::SolvePositionConstraints(b2decimal baumgarte)
{
...
b2Vec2 b2FrictionJoint::GetReactionForce(b2decimal inv_dt) const
{
	return inv_dt * m_linearImpulse;
}

b2decimal b2FrictionJoint::GetReactionTorque(b2decimal inv_dt) const
{
	return inv_dt * m_angularImpulse;
}

void b2FrictionJoint::SetMaxForce(b2decimal force)
{
	b2Assert(b2IsValid(force) && force >= 0.0);
	m_maxForce = force;
}

b2decimal b2FrictionJoint::GetMaxForce() const
{
	return m_maxForce;
}

void b2FrictionJoint::SetMaxTorque(b2decimal torque)
{
	b2Assert(b2IsValid(torque) && torque >= 0.0);
	m_maxTorque = torque;
}

b2decimal b2FrictionJoint::GetMaxTorque() const
{
	return m_maxTorque;
}
