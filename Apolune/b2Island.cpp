...
void b2Island::Solve(const b2TimeStep& step, const b2Vec2& gravity, bool allowSleep)
{
	...
	for (int i = 0; i < m_bodyCount; ++i)
	{
		...
		// Taylor expansion:
		// v2 = (1.0 - c * dt) * v1
		b->m_linearVelocity *= b2Clamp(1.0 - step.dt * b->m_linearDamping, 0.0, 1.0);
		b->m_angularVelocity *= b2Clamp(1.0 - step.dt * b->m_angularDamping, 0.0, 1.0);
	}
	...
	for (int i = 0; i < m_bodyCount; ++i)
	{
		...
		{
			b2decimal ratio = b2_maxTranslation / translation.Length();
			b->m_linearVelocity *= ratio;
		}

		b2decimal rotation = step.dt * b->m_angularVelocity;
		if (rotation * rotation > b2_maxRotationSquared)
		{
			b2decimal ratio = b2_maxRotation / b2Abs(rotation);
	...
	if (allowSleep)
	{
		b2decimal minSleepTime = b2_maxFloat;

		const b2decimal linTolSqr = b2_linearSleepTolerance * b2_linearSleepTolerance;
		const b2decimal angTolSqr = b2_angularSleepTolerance * b2_angularSleepTolerance;
