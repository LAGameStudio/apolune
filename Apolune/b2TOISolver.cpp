...
bool b2TOISolver::Solve(b2decimal baumgarte)
{
	...
	for (int i = 0; i < m_count; ++i)
	{
		...
		b2decimal massA = bodyA->m_mass;
		b2decimal massB = bodyB->m_mass;
		...
		b2decimal invMassA = massA * bodyA->m_invMass;
		b2decimal invIA = massA * bodyA->m_invI;
		b2decimal invMassB = massB * bodyB->m_invMass;
		b2decimal invIB = massB * bodyB->m_invI;
