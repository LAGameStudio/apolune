...
void b2RevoluteJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_enableMotor || m_enableLimit)
	{
		...
		b2Assert(b1->m_invI > 0.0 || b2->m_invI > 0.0);
	}
	...
	b2decimal m1 = b1->m_invMass, m2 = b2->m_invMass;
	b2decimal i1 = b1->m_invI, i2 = b2->m_invI;
	...
	if (m_enableLimit)
	{
		b2decimal jointAngle = b2->m_sweep.a - b1->m_sweep.a - m_referenceAngle;
...
void b2RevoluteJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	{
		b2decimal Cdot = w2 - w1 - m_motorSpeed;
		b2decimal impulse = m_motorMass * (-Cdot);
		b2decimal oldImpulse = m_motorImpulse;
		b2decimal maxImpulse = step.dt * m_maxMotorTorque;
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		{
			b2decimal newImpulse = m_impulse.z + impulse.z;
			if (newImpulse < 0.0)
			{
...
bool b2RevoluteJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	b2decimal angularError = 0.0;
	b2decimal positionError = 0.0;

	// Solve angular limit constraint.
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		b2decimal angle = b2->m_sweep.a - b1->m_sweep.a - m_referenceAngle;
		b2decimal limitImpulse = 0.0;

		if (m_limitState == e_equalLimits)
		{
			// Prevent large angular corrections
			b2decimal C = b2Clamp(angle - m_lowerAngle, -b2_maxAngularCorrection, b2_maxAngularCorrection);
			limitImpulse = -m_motorMass * C;
			angularError = b2Abs(C);
		}
		else if (m_limitState == e_atLowerLimit)
		{
			b2decimal C = angle - m_lowerAngle;
			angularError = -C;
		...
		{
			b2decimal C = angle - m_upperAngle;
			angularError = C;
	...
	// Solve point-to-point constraint.
	{
		...
		b2decimal invMass1 = b1->m_invMass, invMass2 = b2->m_invMass;
		b2decimal invI1 = b1->m_invI, invI2 = b2->m_invI;

		// Handle large detachment.
		const b2decimal k_allowedStretch = 10.0 * b2_linearSlop;
		if (C.LengthSquared() > k_allowedStretch * k_allowedStretch)
		{
			// Use a particle solution (no rotation).
			b2Vec2 u = C; u.Normalize();
			b2decimal m = invMass1 + invMass2;
			if (m > 0.0)
			{
				m = 1.0 / m;
			}
			b2Vec2 impulse = m * (-C);
			const b2decimal k_beta = 0.5f;
...
b2Vec2 b2RevoluteJoint::GetReactionForce(b2decimal inv_dt) const
{
	b2Vec2 P(m_impulse.x, m_impulse.y);
	return inv_dt * P;
}

b2decimal b2RevoluteJoint::GetReactionTorque(b2decimal inv_dt) const
{
	return inv_dt * m_impulse.z;
}

b2decimal b2RevoluteJoint::GetJointAngle() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
...
b2decimal b2RevoluteJoint::GetJointSpeed() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
...
b2decimal b2RevoluteJoint::GetMotorTorque() const
{
	return m_motorImpulse;
}

void b2RevoluteJoint::SetMotorSpeed(b2decimal speed)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
	m_motorSpeed = speed;
...
void b2RevoluteJoint::SetMaxMotorTorque(b2decimal torque)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
...
b2decimal b2RevoluteJoint::GetLowerLimit() const
{
	return m_lowerAngle;
}

b2decimal b2RevoluteJoint::GetUpperLimit() const
{
	return m_upperAngle;
}

void b2RevoluteJoint::SetLimits(b2decimal lower, b2decimal upper)
{
