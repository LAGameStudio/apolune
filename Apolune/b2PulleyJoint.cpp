				...
				b2decimal r)
{
	...
	b2decimal C = lengthA + ratio * lengthB;
...
void b2PulleyJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	b2decimal length1 = m_u1.Length();
	b2decimal length2 = m_u2.Length();
	...
	b2decimal C = m_constant - length1 - m_ratio * length2;
	if (C > 0.0)
	{
	...
	b2decimal cr1u1 = b2Cross(r1, m_u1);
	b2decimal cr2u2 = b2Cross(r2, m_u2);
...
void b2PulleyJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		b2decimal Cdot = -b2Dot(m_u1, v1) - m_ratio * b2Dot(m_u2, v2);
		b2decimal impulse = m_pulleyMass * (-Cdot);
		b2decimal oldImpulse = m_impulse;
	...
	if (m_limitState1 == e_atUpperLimit)
	{
		...
		b2decimal Cdot = -b2Dot(m_u1, v1);
		b2decimal impulse = -m_limitMass1 * Cdot;
		b2decimal oldImpulse = m_limitImpulse1;
	...
	if (m_limitState2 == e_atUpperLimit)
	{
		...
		b2decimal Cdot = -b2Dot(m_u2, v2);
		b2decimal impulse = -m_limitMass2 * Cdot;
		b2decimal oldImpulse = m_limitImpulse2;
...
bool b2PulleyJoint::SolvePositionConstraints(b2decimal baumgarte)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		b2decimal impulse = -m_pulleyMass * C;
	...
	if (m_limitState1 == e_atUpperLimit)
	{
		...
		b2decimal C = m_maxLength1 - length1;
		linearError = b2Max(linearError, -C);
		C = b2Clamp(C + b2_linearSlop, -b2_maxLinearCorrection, 0.0);
		b2decimal impulse = -m_limitMass1 * C;
	...
	if (m_limitState2 == e_atUpperLimit)
	{
		...
		b2decimal C = m_maxLength2 - length2;
		linearError = b2Max(linearError, -C);
		C = b2Clamp(C + b2_linearSlop, -b2_maxLinearCorrection, 0.0);
		b2decimal impulse = -m_limitMass2 * C;
...
b2Vec2 b2PulleyJoint::GetReactionForce(b2decimal inv_dt) const
{
	b2Vec2 P = m_impulse * m_u2;
	return inv_dt * P;
}

b2decimal b2PulleyJoint::GetReactionTorque(b2decimal inv_dt) const
{
	B2_NOT_USED(inv_dt);
	return 0.0;
}
...
b2decimal b2PulleyJoint::GetLength1() const
{
...
b2decimal b2PulleyJoint::GetLength2() const
{
...
b2decimal b2PulleyJoint::GetRatio() const
{
	return m_ratio;
}
